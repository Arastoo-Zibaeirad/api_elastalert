...
class ElastAlerter(object):
    ...
    def get_aggregation_query(self, query, rule, query_key, terms_size, timestamp_field='@timestamp'):
        ...
        print("\n\n\nget_aggregation_query2: ", aggs_query)
        return aggs_query
        ...
        if 'filter' or 'kql' in rule:
            ...
            if int(res['hits']['total']['value']) == 0:
                ...
                if 'events' in res['hits']:
                    if len(res['hits']['events']) == 0:
                        return '1969-12-30T00:00:00Z'        
                    return res['hits']['events'][0][timestamp_field]
                elif 'sequences' in res['hits']:
                    if len(res['hits']['sequences'][0]['events']) == 0:
                        return '1969-12-30T00:00:00Z'        
                    return res['hits']['sequences'][0]['events'][timestamp_field]
            return res['hits']['events'][0][timestamp_field]
